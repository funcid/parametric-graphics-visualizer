...
public class TextAreaFactory {
    ...
    public TextArea[] createNumberTextAreas(int xProperty, int yProperty, int zProperty, boolean isNormal) {
        ...
        // Создание текстовых областей на основе заданных свойств и флага
    ...
    private void setupTextAreaProperties(
            TextArea textArea,
            int value,
            int xProperty,
            int yProperty,
        ...
        // Установка позиции и размера текстовой области на основе переданных свойств и значения
        ...
        // Если это нормальная текстовая область, производим дополнительные настройки
        if (isNormal) {
            textArea.translateYProperty().set(value * TEXT_AREA_TRANSLATE_FACTOR * yProperty - TEXT_AREA_TRANSLATE_FACTOR / 2);
            textArea.setRotationAxis(Rotate.X_AXIS);
            textArea.setRotate(270);
        }

        // Настройка размеров, шрифта и других свойств текстовой области
